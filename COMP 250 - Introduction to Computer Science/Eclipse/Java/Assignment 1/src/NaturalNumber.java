...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
		...
		for(int i = 0; i < firstClone.coefficients.size(); i++) {
			digitSum = firstClone.coefficients.get(i) + secondClone.coefficients.get(i) + carry;
			sum.coefficients.add( digitSum % base );
			if(digitSum >= base)
				carry = 1;
			else
				carry = 0;
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		for(int i = 0; i < this.coefficients.size(); i++) {
			row = multiplicand.timesSingleDigit( this.coefficients.get(i) );

			if(i > 0) {
				for(int j = 0; j < i; j++)
					row.coefficients.addFirst(0);
			}
	...
	public NaturalNumber timesSingleDigit(int multiplierDigit) {
		NaturalNumber product = new NaturalNumber(this.base);
		int digitProd, carry = 0;

		for(int i = 0; i < this.coefficients.size(); i++) {
			digitProd = this.coefficients.get(i) * multiplierDigit + carry;
			product.coefficients.add(digitProd % base);
			if(digitProd >= base)
				carry = digitProd / base;
			else
				carry = 0;
		}
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		int digitDiff, carry = 0;

		for(int i = 0; i < first.coefficients.size(); i++) {
			digitDiff = first.coefficients.get(i) - secondClone.coefficients.get(i) - carry;
			difference.coefficients.add( (digitDiff + base) % base );
			if(digitDiff < 0)
				carry = 1;
			else
				carry = 0;
	...
	public NaturalNumber divide( NaturalNumber  divisor ) throws Exception{
		...
		if(this.compareTo(divisor) == -1)
			...
			while(remainder.compareTo(divisor) == 1 || remainder.compareTo(divisor) == 0) {
				NaturalNumber tempDivisor = divisor.clone();
				NaturalNumber tempQuotient = new NaturalNumber(this.base);
				
				int basePower = 0;
				while(tempDivisor.compareTo(remainder) == -1) {
					tempDivisor.coefficients.addFirst(0);
					basePower++;
				}
				if( tempDivisor.compareTo(remainder) == 1) {
					tempDivisor.coefficients.removeFirst();
					basePower--;
				}
				
				int largestDigitValue = 1;
				for(int i = 1; i < this.base; i++) {
					if(tempDivisor.timesSingleDigit(i).compareTo(remainder) == -1)
						largestDigitValue = i;
					if(i > largestDigitValue)
						break;
				}
				tempDivisor = tempDivisor.timesSingleDigit(largestDigitValue);
				
				tempQuotient.coefficients.add(largestDigitValue);
				tempQuotient.timesBaseToThePower(basePower);
				
				quotient = quotient.plus(tempQuotient);
