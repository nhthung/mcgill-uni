...
public class WordTree
{
	...
	public void insert(String word)
	{
		...
		WordTreeNode endNode = new WordTreeNode();
		endNode = getPrefixNode(word);

		for(int i = endNode.depth; i < word.length(); i++){
			endNode.createChild(word.charAt(i));
			endNode = endNode.children[word.charAt(i)];
		}
		endNode.endOfWord = true;
	...
	WordTreeNode getPrefixNode(String word)
	{
		...
		WordTreeNode prefixNode = new WordTreeNode();
		prefixNode = root;

		for(int i = 0; i < word.length() && prefixNode.children[word.charAt(i)] != null; i++)
			prefixNode = prefixNode.children[word.charAt(i)];
	...
	public boolean contains(String word)
	{
		...
		return getPrefixNode(word).endOfWord;
	...
	public ArrayList<String> getListPrefixMatches( String prefix )
	{
		...
		if(cur != root || (cur == root && prefix.length() == 0)){
			...
			while(!s.empty()){
				...
				if(cur.endOfWord)
					listPrefixMatches.add(cur.toString());

				for(WordTreeNode child : cur.children){
					if(child != null)
						s.push(child);
				}
			}
	...
	public class WordTreeNode
	{
		...
		public String toString()
		{
			...
			Stack<Character> prefixChars = new Stack<Character>();
			WordTreeNode temp = new WordTreeNode();
			String prefix = "";
			temp = this;

			while(temp.charInParent != (char) 0){
				prefixChars.push(temp.charInParent);
				temp = temp.parent;
			}

			while(!prefixChars.empty())
				prefix += prefixChars.pop();
