...
void update_pod(
     ...
     * Also check if new entry will overwrite an old one,
     * if so update linked list of entries which have the same key as the
     * entry which will be overwritten
     */
    if (!after_write) {
        ...
        /* If pod is full, the new entry will overwrite an old one.
         * Have to update linked list of key associated with overwritten entry
         * so the read function works correctly
         */
        if (pod->size == POD_CAPACITY) {
            int i;
            for (
                i = pod->next[entry_index];
                pod->next[i] != entry_index;
                i = pod->next[i]);

            pod->next[i] = pod->next[entry_index];
        }
    }
     ...
     */
        ...
        if (next_index > -1) {
            ...
            /* Update linked list */
            if (pod->next[next_index] != entry_index) {
                pod->next[entry_index] = pod->next[next_index];
                pod->next[next_index] = entry_index;
            }
            else {
                pod->next[entry_index] = next_index;
            }


            /* Set id of key to match with id identical key which was
             * inserted before
             */
            entry->id = pod->entries[next_index].id;
        }
        else {
            /* If is sole entry with unique key in pod, then
             * set id of key to be different from all the rest
             */
